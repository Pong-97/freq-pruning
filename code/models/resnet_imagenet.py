...
class ResNet(nn.Module):
    def __init__(self, block, num_layers, num_blocks, covcfg, cfg, num_classes=1000):
        ...
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1, compress_rate=compress_rate[0 : 3*num_blocks[0]], tmp_name='layer1')
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2, compress_rate=compress_rate[3*num_blocks[0] : 3*num_blocks[0]+3*num_blocks[1]], tmp_name='layer2')
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2, compress_rate=compress_rate[3*num_blocks[0]+3*num_blocks[1]:3*num_blocks[0]+3*num_blocks[1]+3*num_blocks[2]], tmp_name='layer3')
        self.layer4 = self._make_layer(block, 512, num_blocks[3], stride=2, compress_rate=compress_rate[3*num_blocks[0]+3*num_blocks[1]+3*num_blocks[2]:], tmp_name='layer4')
...
def main():
    model = resnet_50()
    # model = resnet_50(cfg=[32, 32, 128, 32, 32, 128, 32, 32, 128, 64, 64, 256, 64, 64, 256, 64, 64, 256, 64, 64, 256, 128, 128, 512, 128, 128, 512, 128, 128, 512, 128, 128, 512, 128, 128, 512, 128, 128, 512, 256, 256, 1024, 256, 256, 1024, 256, 256, 1024])
    # model = resnet_50(cfg=[34, 34, 34, 136, 34, 34, 136, 34, 34, 136, 68, 68, 273, 68, 68, 273, 68, 68, 273, 68, 68, 273, 136, 136, 547, 136, 136, 547, 136, 136, 547, 136, 136, 547, 136, 136, 547, 136, 136, 547, 273, 273, 1095, 273, 273, 1095, 273, 273, 1095])
