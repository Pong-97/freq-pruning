...
parser.add_argument('--student_epochs', type=int, default=30, metavar='N',
...
parser.add_argument('--save', default='/home2/pengyifan/pyf/freq-lite/code/Ablation_expriment/3/vgg/contract' , type=str, metavar='PATH',
...
def save_checkpoint(state, is_best, filepath, finetune=0):
    if finetune == 0:
...
def student_kd_main(student_kd_model, cfg, teacher_model, train_loader, test_loader):
...
def main():
    ...
    # tar_cfg = [40, 40, 'M', 60, 60, 'M', 90, 90, 90, 'M', 210, 210, 210, 'M', 210, 210, 210, 210]
    # FLOPS:  62843544.0 
    # PARAMS: 2537856.0
    tar_cfg = [48, 48, 'M', 96, 96, 'M', 96, 96, 96, 'M', 200, 200, 200, 'M', 200, 200, 200, 200]
    # FLOPS:  89583520.0 
    # PARAMS: 2482474.0 
    ori_cfg = [64, 64, 'M', 128, 128, 'M', 256, 256, 256, 'M', 512, 512, 512, 'M',512, 512, 512]
    conv_idx = 0
    print(tar_cfg)
    for i in range(len(ori_cfg)):
        print("###################  layer index:{}  ###################".format(i+1))
        # layer_compress_rate = (ori_cfg[i]-tar_cfg[i])/ori_cfg[i]
        if ori_cfg[i] == 'M':
            print("Pooling layer\n")
            continue
        layer_keep_rate = tar_cfg[i]/ori_cfg[i]
        ori_cfg[i] = tar_cfg[i]
        # masks, new_cfg, oldmodel, newmodel = hypergraghcluster(cfg=cfg, layer_idx=i, modelpath=os.path.join(args.save, 'model_best.pth.tar'))
        cfg, newmodel = processed_forward(os.path.join(args.save, 'model_best.pth.tar'), layer_keep_rate, layer_idx=conv_idx)
        student_kd_model = newmodel
        student_kd_model, student_kd_history = student_kd_main(student_kd_model, cfg, teacher_model, train_loader, test_loader)
        conv_idx = conv_idx + 1
    ...
    test_model = vgg_16_bn(cfg=checkpoint['cfg'])
    test_model.cuda()
