...
from freq_process_cifar_block import processed_forward
...
parser.add_argument('--save', default="/home2/pengyifan/pyf/freq-lite/logs/resnet110/405060" , type=str, metavar='PATH',
...
def main():
    ...
    compress_rate = [0.4]*37 + [0.5]*36 + [0.6]*36
    ...
    for i in range(54):
        print("##############################################################################")
        if i == 0:
            keep_rate = [1-compress_rate[0], 1-compress_rate[1], 1-compress_rate[2]]
        else:
            keep_rate = [1-compress_rate[2*i+1], 1-compress_rate[2*i+2]]
        cfg, newmodel = processed_forward(os.path.join(args.save, 'model_best.pth.tar'), keep_rate, block_idx=i, model_arch='resnet110')         
    ...
    # train_layer_idx = checkpoint['block_idx'] + 1 #TODO:要不要加1?

    # for i in range(train_layer_idx):
    #     cfg[i] = finished_cfg[i]
    # for i in range(train_layer_idx,len(cfg)):
    #     cfg[i] = tar_cfg[i]
    #     masks, new_cfg, oldmodel, newmodel = hypergraghcluster(cfg=cfg, layer_idx=i, modelpath=os.path.join(args.save, 'model_best.pth.tar'), model='resnet56')
    #     newmodel = transfer_value(oldmodel, newmodel, masks)
    #     student_kd_model = newmodel
    #     print("layer index: ", i+1)
    #     student_kd_model, student_kd_history = student_kd_main(student_kd_model, new_cfg, teacher_model, train_loader, test_loader, layer_idx=i)
    # # ##########################################################################  

    print("=> test, loading student model checkpoint '{}'".format(os.path.join(args.save, 'model_best.pth.tar')))
    checkpoint = torch.load(os.path.join(args.save, 'model_best.pth.tar'), map_location='cpu')
    print(checkpoint['cfg'])
    test_model = resnet_110(cfg=checkpoint['cfg'])
    test_model.cuda()
