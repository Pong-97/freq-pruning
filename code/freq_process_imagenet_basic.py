...
from models.resnet_imagenet_basic import ResBasicBlock
from data_process import CIFAR10_DATA_PROCESS, CIFAR100_DATA_PROCESS, MNIST_DATA_PROCESS
from models import *
from models.resnet_imagenet_basic import resnet18
...
def processed_forward(model_path, keep_ratio, dataset='imagenet', seg_num=8, block_idx=0, model_arch='resnet50', ):
    ...
    for k, m in enumerate(model.modules()):
        if isinstance(m, nn.BatchNorm2d) and (k not in [29, 45, 61]) and k>3:  
            # pdb.set_trace()
            cfg.append(m.weight.shape[0])
            cfg_mask.append(torch.ones(m.weight.shape[0]))

    cfg[2*block_idx] = cfg1
    cfg[2*block_idx+1] = cfg2

    cfg_mask[2*block_idx] = cfg1_mask[0]
    cfg_mask[2*block_idx+1] = cfg2_mask[0]
...
def transfer_value(cfg_mask, model, newmodel, block_idx):
    ...
    for k, [m0, m1] in enumerate(zip(model.modules(), newmodel.modules())):
        ...
        if k in [28, 45, 59]:
            # pdb.set_trace()
            if (k==28 and block_idx==2) or (k==45 and block_idx==4) or (k==59 and block_idx==6):
                continue
            else:
            ...
            # pdb.set_trace()
            if (k==29 and block_idx==2) or (k==46 and block_idx==4) or (k==60 and block_idx==6):
                continue
            else:
...
def prepare_model(model_path='', layer_idx=0, model_arch='resnet50'):
    ...
    if model_arch=='resnet18':
        if layer_idx == 0:
            ...
            model_head = resnet18()
            model_tail = resnet18()
            ori_model_path = "/home2/pengyifan/pyf/freq/pretrained_model/resnet18-5c106cde.pth"
            if os.path.isfile(ori_model_path):
                ...
                model_head.load_state_dict(checkpoint, strict=False)
                model_tail.load_state_dict(checkpoint, strict=False)
                model.cuda()
            ...
            model = resnet18(cfg=model_cfg)
            model_head = resnet18(cfg=model_cfg)
            model_tail = resnet18(cfg=model_cfg)
            model.cuda()
            model = nn.DataParallel(model) 
            model_head.cuda()
            model_head = nn.DataParallel(model_head) 
