...
from freq_process_imagenet_basic import processed_forward
...
def main():
    ...
    tar_cfg = []
    cfg =  [64, 64, 64, 64, 128, 128, 128, 128, 256, 256, 256, 256, 512, 512, 512, 512]
    # [40, 40, 40, 40, 80, 80, 80, 80, 180, 180, 180, 180, 400, 400, 400, 400]
    compress_rate = [0.375]*4 + [0.375]*4 + [0.3]*4 + [0.22]*4
    ...
    for i in range(8):
        print("##############################################################################")
        keep_rate = [1-compress_rate[2*i], 1-compress_rate[2*i+1]]
        cfg, newmodel = processed_forward(os.path.join(args.save, 'model_best.pth.tar'), keep_rate, block_idx=i, model_arch='resnet18')         
    ...
    test_model = resnet18(cfg=checkpoint['cfg'])
    test_model.cuda()
